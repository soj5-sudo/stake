...
// ── ONE-RENDER GATE ──
// First render runs free. Any further generate (hero / main / regen / workshop)
// shows an alert; OK redirects the user to jewellabs.org.
const RenderGate = {
  KEY: 'vltava_render_used',
  REDIRECT: 'https://jewellabs.org',
  used() {
    try { return localStorage.getItem(this.KEY) === '1'; } catch (e) { return false; }
  },
  mark() {
    try { localStorage.setItem(this.KEY, '1'); } catch (e) { /* ignore */ }
  },
  // Returns true if the caller should proceed with generation,
  // false if the gate has triggered (and handled the redirect).
  check() {
    if (this.used()) {
      alert('One free render. Click OK to continue to Jewel Labs.');
